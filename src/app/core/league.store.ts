...
import { Formation, MAX_HANDLERS_HORIZONTAL_STACK, MAX_HANDLERS_VERTICAL_STACK, Position } from './models/formation.enum';
...
export const LeagueStore = signalStore(
  ...
  withComputed(({ playerEntities, userTeamPlayersIds }) => ({
    userTeamPlayer: computed(() => {
      return userTeamPlayersIds().map(id => playerEntities().find(player => player.id === id))
    }),
  })),
  withComputed(({ formation, userTeamId, userTeamPlayer }) => ({
    playersWithPosition: computed(() => {
      console.log(formation(), userTeamId());
      const teamPlayers = userTeamPlayer().filter(player => (player as Player).teamId === userTeamId());
      if (formation() === Formation.Hex) {
        ...
        return teamPlayers.map((el, index) =>
          ({ ...el, position: index < MAX_HANDLERS_HORIZONTAL_STACK ? Position.Handler : Position.Cutter }))
      } else if (formation() === Formation.VerticalStack) {
        return teamPlayers.map((el, index) =>
          ({ ...el, position: index < MAX_HANDLERS_VERTICAL_STACK ? Position.Handler : Position.Cutter }))
      }
      return teamPlayers
    }),
  })),
  withComputed(({ userTeamId, playerEntities }) => ({
    playerWithAvailability: computed(() => playerEntities().map(player => addPlayerAvailability(player, userTeamId()))),
  })),
  withMethods((store) => ({
    ...
    swapPlayers: (player1Id: number, player2Id: number) => {
      ...
      console.log(players);
      return patchState(store, (state) => ({ ...state, userTeamPlayersIds: players }));
    }
  }))
);
